...
[corCAR1()](https://goo.gl/Zhbzfc), [corExp()](https://goo.gl/CfU91s) and 
...
correlation structures. These models are not nested and therfore we can only compare them
...
To investigate the covariance structuer we use function 
[getVarCov()](https://goo.gl/XqY8K7) which returns the estimate covariance matrix. Due to
the fact that patients have different number of repeated measuements, argument 
`individual` of [getVarCov()](https://goo.gl/XqY8K7) can be used to specify for which 
subject we wisk to inspect the fitted covariance matrix. In the following example we 
