...
freedom we need use function [ns()](https://goo.gl/0g115B). Hence, the call to
...
We observe that the compound symmetry model `fm_2` has the lowest AIC and BIC values. 
Next we compare the heteroscedastic models:
```{r}
anova(fm_6, fm_7, fm_8, fm_9)
```

Again the heteroscedastic compound symmetry model `fm_6` is the winner. Finally, we 
...
The result suggests that the heteroscedastic compound symmetry model fits the data better.
...
terms that we wish to test are `ns(year, 3):sex` and `ns(year, 3):drug`. Using the 
`Terms` argument of the [anova()](https://goo.gl/RuqkSV) function we perform the 
corresponding F-test as we have seen in Section 2.9:
```{r}
...
To peform the likelihood ratio test we need to give the model under the null `fm_6_noInt`
and the model under the alternative hypothesis `fm_6` to the 
...
ratio test, we need to fit the models using Maximum Likelihood (ML) and Restricted Maximum 
...
virtually the same p-value as in the F-test.
...
estimation metod the maximum likelihood. Then we use the 
...
The results suggest that we could continue by assuming that average prothrombin time has a
linear time evolution.

### Question 7
Using the [summary()](https://goo.gl/9ymFa6) function we obtain a detailed output of our 
final model `fm_6ML_noIntLin`
```{r}
...
year the average prothrombin time increases by `r round(coef(fm_6ML_noIntLin)['year'], 2)`
sec.

* Female patients have on average a prothrombin time 
`r abs(round(coef(fm_6ML_noIntLin)['sexfemale'], 2))` sec less than their male 
...
`r abs(round(coef(fm_6ML_noIntLin)['drugD-penicil'], 2))` sec less than the placebo 
...
`r abs(round(coef(fm_6ML_noIntLin)['age'], 2))` sec for patients of the same sex, who 
...
From the diagonal elements we observe that variance of the error terms increases quite 
fast in time. To transform the covariance to correlation we use function 
