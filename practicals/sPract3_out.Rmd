...
interaction terms in the model. In the framework of GEE models we do not pefrom likelihood
...
We observe that the result is on the bordeline. Nonetheless, we will continue to 
investigate the interaction terms further. We will first test the two interaction terms 
for the time effect, i.e.,
```{r}
fm_3_IntTime <- geeglm(Dicht_prothro ~ ns(year, 2) + sex + drug + age + age : sex, 
                       family = binomial(), data = pbc2, id = id, corstr = "ar1")

anova(fm_3_IntTime, fm_3)
```
...
for drug, i.e.,
```{r}
fm_3_IntDrug <- geeglm(Dicht_prothro ~ ns(year, 2) * sex + sex + age, 
                       family = binomial(), data = pbc2, id = id, corstr = "ar1")
