...
For our example, we want to fit a zero-inflated negative binomial mixed model with first specifying only fixed effects for the logistic regression for the first latent class. This is achieved with the following syntax:
```{r ZI_Netative_Binomial}
fm3 <- update(fm2, family = zi.negative.binomial(),
              zi_fixed = ~ drug + obstime)
```

We standard negative binomial model is nested within the zero-inflated negative binomial one, and therefore we can compare the two using a likelihood ratio test:
```{r LRT_NB_ziNB}
anova(fm2, fm3)
```

The results suggest that the frequency of zero counts is greater than the one assumed by the standard negative binomial model. We continue to see if include a random intercept in the logistic regression for the first latent class further improves the fit; because the number of random effects has increased considerably we set the number of quadrature points to five:
```{r ZI_Netative_Binomial2}
fm4 <- update(fm3, zi_random = ~ 1 | patient, nAGQ = 5)
```
